    ...
    <div className="max-w-3xl mx-auto py-8 px-4 pb-24 md:pb-12 space-y-8 animate-in fade-in duration-200 font-sans">
      ...
      <div className="liquid-glass-card border border-slate-700/80 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left z-10 relative">
          ...
          <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl bg-slate-800 border-4 border-slate-700 flex items-center justify-center text-6xl shadow-2xl shrink-0">
            {userAvatar || '🧑‍🎓'}
          </div>
      ...
      {/* Daily Goal Status */}
      <div className="liquid-glass-strong border border-white/10 rounded-3xl p-6 space-y-4 shadow-2xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-tr from-amber-500 to-yellow-500 flex items-center justify-center shadow-lg">
              <Trophy className="w-6 h-6 text-white" />
            </div>
            <h3 className="font-black text-xl text-white">Meta Diaria de Estudio</h3>
          </div>
          <span className="text-sm font-black text-emerald-400 bg-emerald-500/20 px-3 py-1 rounded-xl border border-emerald-500/30">{Math.min(userXp, goal)} / {goal} XP</span>
        </div>

        <div className="h-5 bg-slate-950 rounded-full overflow-hidden p-1 shadow-inner border border-slate-800">
          <div
            className="bg-gradient-to-r from-emerald-500 via-teal-400 to-cyan-400 h-full rounded-full transition-all duration-500 shadow-md"
            style={{ width: `${Math.min((userXp / goal) * 100, 100)}%` }}
          />
        </div>

        <p className="text-xs text-slate-300 font-bold">
          {userXp >= goal ? '🎉 ¡Felicidades! Completaste tu meta diaria en el taller.' : '🔥 ¡Sigue encadenando aciertos para completar tu objetivo y encender tu combo!'}
        </p>
      </div>
